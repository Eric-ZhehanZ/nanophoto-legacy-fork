...
const addUpload = async ({
  ...
  if (formDataFromExif) {
    ...
    const form: Partial<PhotoFormData> = {
      ...formDataFromExif,
      title: title || aiTitle,
      titleZh, captionZh, semanticDescriptionZh: semanticZh,
  ...
  runAuthenticatedAdminServerAction(async () => {
    ...
    if (photo) {
      ...
      if (formDataFromExif) {
        ...
        const {
          title: atTitle, titleZh, captionZh, semanticZh,
          caption: aiCaption,
          tags: aiTags,
          ...
          existingSemantic: photo.semanticDescription,
          uniqueTags,
        });
        ...
        const photoFormDbInsert =
          await convertFormDataToPhotoDbInsertAndLookupRecipeTitle({
            ...
            titleZh: photo.titleZh || titleZh,
            captionZh: photo.captionZh || captionZh,
            semanticDescriptionZh: photo.semanticDescriptionZh || semanticZh,
...
// One authenticated structured request supplies both languages atomically.
export const generateBilingualMetadataAction = async (
  imageBase64: string,
  fields: import('./ai').AiAutoGeneratedField[],
) => runAuthenticatedAdminServerAction(async () => {
  const result = await generateAiImageQueries({
    imageBase64, textFieldsToGenerate: fields, uniqueTags: [],
  });
